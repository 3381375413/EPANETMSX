...
static void gone(MSXResidentGpu*g)
{
    ...
    cudaFree(g->daVol);cudaFree(g->daHyd);cudaFree(g->daH);cudaFree(g->daNf);
    ...
    freePinned(g->haErr);freePinned(g->haVol);freePinned(g->haHyd);
