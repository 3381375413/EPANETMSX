...
/* Allocation-free iterator over the authoritative used slots.  ``begin``
   resets the cursor, ``validate`` performs the complete legacy enumerate
   checks without materializing rows, ``count`` returns its result, and
   ``next`` emits one row in the same link/descriptor order. */
typedef struct {
 uint32_t linkIndex,slot,seen,base,count,total,expectedCount,nlinks;
 int32_t orient;
 int active,done,validated;
} MSXResidentActiveIterator;
...
MSXResidentStatus MSXresident_enumerateActive(MSXResidentActiveRow *rows,
                                              uint32_t capacity,
                                              uint32_t *count);
MSXResidentStatus MSXresident_beginActiveIterator(MSXResidentActiveIterator *);
MSXResidentStatus MSXresident_validateActiveIterator(MSXResidentActiveIterator *);
MSXResidentStatus MSXresident_countActiveIterator(const MSXResidentActiveIterator *,
                                                   uint32_t *count);
